...
Use existing tutorials as living examples, not as a fixed template. Always read the `47-cuda-events` and `49-hid` English/Chinese pairs, then let the agent select one to three additional completed tutorial pairs whose subsystem, attachment type, tool shape, or target reader matches the new lesson. Read every selected file in full and record why it was chosen. Never infer house style from headings alone.
...
Reject a draft until it satisfies every non-diagnostic rule in the three references. Report each failure with the file, heading, smallest useful excerpt, reason, and a targeted correction. Never rewrite a whole section during review when a paragraph-level fix is enough.
...
Use `scripts/sync-source-blocks.py` for every complete-source block. Mark each block with a repository-relative source path:
...
Run the script with `--write` after drafting and `--check` before acceptance. Never hand-edit content between these markers.

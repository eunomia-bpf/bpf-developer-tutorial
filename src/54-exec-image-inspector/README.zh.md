...
本教程构建一个持续运行、在内核层面捕获这些信息的工具。它在每次成功 exec 提交凭据后触发钩子，安排一个延迟回调，然后读取已安装可执行文件的 ELF 头部，报告其架构、字节序和文件类型。过程中会演示两个较新的内核特性（BPF task work 和 file dynptr），它们组合起来可以解决传统 eBPF 方案无法处理的问题。
